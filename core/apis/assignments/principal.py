...
def list_assignments(p):
    """Return list of all submitted and graded assignments."""
    principal_assignments = Assignment.get_submitted_and_graded_assignments()
    assignment_dump = AssignmentSchema().dump(principal_assignments, many=True)
...
def grade_assignment(p, incoming_payload):
    ...
    if not p:
        return APIResponse.respond(message='Unauthorized', status_code=401)
